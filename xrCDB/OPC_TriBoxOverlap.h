...
inline_ BOOL OBBCollider::TriBoxOverlap()
{
	...
	// Find min, max of the triangle in x-direction, and test for overlap in X
	FINDMINMAX(v0.x, v1.x, v2.x, min_value, max_value);
	if (min_value > mBoxExtents.x || max_value < -mBoxExtents.x)
		return FALSE;

	FINDMINMAX(v0.y, v1.y, v2.y, min_value, max_value);
	if (min_value > mBoxExtents.y || max_value < -mBoxExtents.y)
		return FALSE;

	FINDMINMAX(v0.z, v1.z, v2.z, min_value, max_value);
	if (min_value > mBoxExtents.z || max_value < -mBoxExtents.z)
		return FALSE;
#endif
	// 2) Test if the box intersects the plane of the triangle
	// compute plane equation of triangle: normal*x+d=0
