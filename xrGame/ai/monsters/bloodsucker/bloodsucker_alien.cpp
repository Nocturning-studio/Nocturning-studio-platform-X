...
#define BS_DELTA_ANGLE_X 10 * PI / 180
#define BS_DELTA_ANGLE_Y 10 * PI / 180
#define BS_DELTA_ANGLE_Z 10 * PI / 180
#define BS_ANGLE_SPEED 0.2f

#define BS_MIN_FOV 70.f
#define BS_MAX_FOV 175.f
#define BS_FOV_SPEED 80.f
#define BS_MAX_CAMERA_DIST 3.5f

CAlienEffector::CAlienEffector(ECamEffectorType type, CAI_Bloodsucker* obj) : inherited(type, flt_max)
{
	dangle_target.set(angle_normalizef(Random.randFs(BS_DELTA_ANGLE_X)),
					  angle_normalizef(Random.randFs(BS_DELTA_ANGLE_Y)),
					  angle_normalizef(Random.randFs(BS_DELTA_ANGLE_Z)));
...
BOOL CAlienEffector::ProcessCam(SCamEffectorInfo& info)
{
	...
	// set angle
	if (angle_lerpf(dangle_current.x, dangle_target.x, BS_ANGLE_SPEED, Device.fTimeDelta))
	{
		dangle_target.x = angle_normalizef(Random.randFs(BS_DELTA_ANGLE_X));
	}

	if (angle_lerpf(dangle_current.y, dangle_target.y, BS_ANGLE_SPEED, Device.fTimeDelta))
	{
		dangle_target.y = angle_normalizef(Random.randFs(BS_DELTA_ANGLE_Y));
	}

	if (angle_lerpf(dangle_current.z, dangle_target.z, BS_ANGLE_SPEED, Device.fTimeDelta))
	{
		dangle_target.z = angle_normalizef(Random.randFs(BS_DELTA_ANGLE_Z));
	}
	...
	float rel_dist = m_prev_eye_matrix.c.distance_to(cur_matrix.c) / BS_MAX_CAMERA_DIST;
	...
	float m_target_fov = BS_MIN_FOV + (BS_MAX_FOV - BS_MIN_FOV) * rel_speed;
	def_lerp(m_current_fov, m_target_fov, BS_FOV_SPEED, Device.fTimeDelta);
