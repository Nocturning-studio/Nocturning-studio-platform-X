...
#define MELEE_JUMP_CHECK_YAW 165 * PI / 180
#define MELEE_JUMP_DELAY_MIN 500
#define MELEE_JUMP_DELAY_MAX 1000
#define MELEE_JUMP_MAX_DISTANCE_TO_ENEMY 4.f
...
bool CControlMeleeJump::check_start_conditions()
{
	...
	if (m_man->direction().is_face_target(enemy_position, MELEE_JUMP_CHECK_YAW))
		return false;
	if (enemy_position.distance_to(m_object->Position()) > MELEE_JUMP_MAX_DISTANCE_TO_ENEMY)
		return false;
...
void CControlMeleeJump::activate()
{
	...
	float target_yaw = angle_normalizef(-dir_to_enemy.getH());
	...
	ctrl_data_dir->heading.target_speed = angle_differencef(cur_yaw, target_yaw) / anim_time;
...
void CControlMeleeJump::on_release()
{
	...
	m_time_next_melee_jump =
		Device.dwTimeGlobal + Random.randI(MELEE_JUMP_DELAY_MIN, MELEE_JUMP_DELAY_MAX);
}
