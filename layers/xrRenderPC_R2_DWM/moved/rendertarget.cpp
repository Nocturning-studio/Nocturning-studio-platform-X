...
void CRenderTarget::u_setzb(ID3D11DepthStencilView* zb)
{
	RCache.set_ZB(zb);
...
void CRenderTarget::u_setrt(u32 W, u32 H, ID3D11RenderTargetView* _1, ID3D11RenderTargetView* _2, 
	ID3D11RenderTargetView* _3, ID3D11RenderTargetView* _4)
{
	dwWidth = W;
	dwHeight = H;
...
void	CRenderTarget::u_compute_texgen_jitter	(Fmatrix&		m_Texgen_J)
{
	...
	// rescale - tile it
	float	scale_X			= dwWidth	/ float(TEX_jitter);
	float	scale_Y			= dwHeight/ float(TEX_jitter);
