...
void	CBlender_light_occq::Compile(CBlender_Compile& C)
{
	...
	case 2:			// Stencil clear in case we've ran out of markers.
		...
		if (RImplementation.o.aa_mode == AA_MSAA)
			C.r_Stencil(TRUE, D3D11_COMPARISON_ALWAYS, 0x00, 0x7E, D3DSTENCILOP_ZERO, D3DSTENCILOP_ZERO, D3DSTENCILOP_ZERO);
		else //	Clear all bits except the last one
			C.r_Stencil(TRUE, D3D11_COMPARISON_ALWAYS, 0x00, 0xFE, D3DSTENCILOP_ZERO, D3DSTENCILOP_ZERO, D3DSTENCILOP_ZERO);
		C.r_End		();
		break;
