...
void	CBlender_accum_direct::Compile(CBlender_Compile& C)
{
	...
	case SE_SUN_MIDDLE:		// middle pass - enable Z-test to perform depth-clipping
		//	FVF::TL2uv
		C.r_Pass("accum_v", "accum_sun_near_middle", FALSE, TRUE, FALSE, FALSE, D3DBLEND_ONE, dest);
	...
	case SE_SUN_FAR:		// far pass, only stencil clipping performed
		//	FVF::TL2uv
		C.r_Pass("accum_v", "accum_sun_far", FALSE, TRUE, FALSE, blend, D3DBLEND_ONE, dest);
		C.r_CullMode(D3DCULL_NONE);
		C.r_dx10Texture("s_material", tex_t_material);
		C.r_dx10Texture("s_diffuse", tex_rt_Color);
		C.r_dx10Texture("s_position", tex_rt_Position);
...

		{
			...
			C.i_dx10Address(s, D3D11_TEXTURE_ADDRESS_BORDER);
	...
	case SE_SUN_FAR_VOLUMETRIC:		// volumetric pass
		C.r_Pass("accum_v", "accum_sun_far_volume", FALSE, FALSE, FALSE, TRUE, D3DBLEND_ONE, D3DBLEND_ONE);
		C.r_dx10Texture("s_material", tex_t_material);
		C.r_dx10Texture("s_diffuse", tex_rt_Color);
		C.r_dx10Texture("s_position", tex_rt_Position);
		C.r_dx10Texture("s_depth", tex_t_depth);
	...
	case SE_SUN_FAR_VOL_MINMAX:		// volumetric pass with minmax
		C.r_Pass("accum_v", "accum_sun_far_volume_minmax", FALSE, FALSE, FALSE, TRUE, D3DBLEND_ONE, D3DBLEND_ONE);
		C.r_dx10Texture("s_material", tex_t_material);
		C.r_dx10Texture("s_diffuse", tex_rt_Color);
		C.r_dx10Texture("s_position", tex_rt_Position);
		C.r_dx10Texture("s_depth", tex_t_depth);
	...
	case SE_SUN_VSM:		// enable Z-test to perform depth-clipping
		//	FVF::TL2uv
		C.r_Pass("accum_v", "accum_sun_vsm", FALSE, TRUE, FALSE, FALSE, D3DBLEND_ONE, dest);
		C.r_CullMode(D3DCULL_NONE);
		C.PassSET_ZB(TRUE, FALSE, TRUE);	// force inverted Z-Buffer
		C.r_dx10Texture("s_material", tex_t_material);
		C.r_dx10Texture("s_diffuse", tex_rt_Color);
