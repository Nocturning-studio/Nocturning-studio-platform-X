...
void	CBlender_BmmD::Compile	(CBlender_Compile& C)
{
	...
	case SE_NORMAL_HQ: 		// deffer & forward
		strcpy_s(oR_Name_hack, oR_Name);
		strcpy_s(oG_Name_hack, oG_Name);
		strcpy_s(oB_Name_hack, oB_Name);
		strcpy_s(oA_Name_hack, oA_Name);
		...
		C.r_Stencil(TRUE, D3D11_COMPARISON_ALWAYS, 0xff, 0x7f, D3DSTENCILOP_KEEP, D3DSTENCILOP_REPLACE, D3DSTENCILOP_KEEP);
		C.r_StencilRef(0x01);
