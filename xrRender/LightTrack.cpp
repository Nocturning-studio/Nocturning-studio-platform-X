...
void CROS_impl::update(IRenderable* O)
{
	...
	if (MODE & IRender_ObjectSpecific::TRACE_HEMI)
	{
		...
		{ // five samples per one frame
			...
			// take sample
			Fvector hemi_direction;
			hemi_direction.set(hdir[sample][0], hdir[sample][1], hdir[sample][2]).normalize();
			//.			result[sample]	=
			//! g_pGameLevel->ObjectSpace.RayTest(position,direction,50.f,collide::rqtBoth,&cache[sample],_object);
			result[sample] = !g_pGameLevel->ObjectSpace.RayTest(position, hemi_direction, 50.f, collide::rqtStatic,
