...
void CRender::add_leafs_Dynamic(IRender_Visual* pVisual)
{
	...
	case MT_PARTICLE_GROUP: {
		...
		{
			PS::CParticleGroup::SItem& Item = *i_it;
			if (Item._effect)
				add_leafs_Dynamic(Item._effect);
			for (xr_vector<IRender_Visual*>::iterator pit = Item._children_related.begin();
				 pit != Item._children_related.end(); pit++)
				add_leafs_Dynamic(*pit);
			for (xr_vector<IRender_Visual*>::iterator pit = Item._children_free.begin();
				 pit != Item._children_free.end();
				 pit++)
				add_leafs_Dynamic(*pit);
		}
	}
...
BOOL CRender::add_Dynamic(IRender_Visual* pVisual, u32 planes)
{
	...
	case MT_SKELETON_RIGID: {
		...
		if (pV->m_lod)
		{
			Fvector Tposition;
			float D;
			val_pTransform->transform_tiny(Tposition, pV->vis.sphere.P);
			float ssa = CalcSSA(D, Tposition, pV->vis.sphere.R / 2.f); // assume dynamics never consume full sphere
			if (ssa < r_ssaLOD_A)
				_use_lod = TRUE;
		}
		if (_use_lod)
