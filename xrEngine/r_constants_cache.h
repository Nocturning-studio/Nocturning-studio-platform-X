  ...
  public:
	...
	ICF void access_direct(R_constant* C, u32 DataSize, void** ppVData, void** ppGData, void** ppPData)
	{
		...
		if (ppGData)
		{
			...
			{
				access_direct(C, C->gs, ppGData, DataSize, BT_HullBuffer);
			}
			else
				*ppGData = 0;
		}
		if (ppGData)
		{
			...
			{
				access_direct(C, C->gs, ppGData, DataSize, BT_DomainBuffer);
			}
			else
				*ppGData = 0;
		}
		if (ppGData)
		{
			...
			{
				access_direct(C, C->gs, ppGData, DataSize, BT_Compute);
			}
			else
				*ppGData = 0;
		}
